...
use tower_lsp::lsp_types::{
    ...
    VersionedTextDocumentIdentifier, WorkDoneProgressOptions, WorkspaceEdit, TextDocumentIdentifier,
...
impl LanguageServer for Backend {
    ...
    // Temporary function to return semantic tokens
    async fn document_highlight(
        ...
        let semantic_tokens_result = self.semantic_tokens_full(SemanticTokensParams {
            text_document: TextDocumentIdentifier { uri: uri.clone() },
            work_done_progress_params: Default::default(),
            partial_result_params: Default::default(),
        }).await;
    
        if let Ok(Some(SemanticTokensResult::Tokens(tokens))) = semantic_tokens_result {
            let highlights: Vec<DocumentHighlight> = tokens.data.iter().map(|token| {
                DocumentHighlight {
                    range: Range {
                        start: Position::new(token.delta_line, token.delta_start),
                        end: Position::new(token.token_type, token.length),
                    },
                    kind: None,
                }
            }).collect();
